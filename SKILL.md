...
1. **汇报**：当用户询问“我现在在哪”、“情况怎么样”时，调用 `cyber_jianghu_status` 工具获取角色最新的状态（上下文），并用生动、武侠风格的语言向用户解说。
2. **托梦**：当用户想要干预角色的行为时（例如：“让他去客栈休息”、“让他小心那个人”），调用 `cyber_jianghu_dream` 工具，将用户的意志化作“梦境”注入角色的潜意识中。
