...
- **结构化认知上下文**: `GET /api/v1/cognitive` - 获取四阶段认知数据（感知→动机→规划→决策）。Bootstrap Hook 会自动调用此端点并生成 `CONTEXT.md`。
...
2. **当前环境**：来自 Bootstrap Hook 生成的 `CONTEXT.md`（由 `/api/v1/cognitive` 端点数据驱动）。
...
向 Agent 注入持续若干回合的念头，这会在 `CONTEXT.md` 中置顶显示（通过 `/api/v1/cognitive` 端点获取）。
